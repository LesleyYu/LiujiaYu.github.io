...
async function searchArtist(req, res) {
  ...
  if (!token) {
    throw createError(502, 'Failed to obtain Artsy token');
  }
...
async function getArtistDetail(req, res) {
  ...
  if (!response.ok) {
    ...
    throw createError(response.status, `Failed to fetch artist detail: ${errorText}`);
  }
  const data = await response.json();
  const filteredData = {
    id: data.id,
...
module.exports = {
  searchArtist: asyncHandler(searchArtist),
  getArtistDetail: asyncHandler(getArtistDetail),
  getSimilarArtists: asyncHandler(getSimilarArtists),
  getArtworks: asyncHandler(getArtworks),
  getGenes: asyncHandler(getGenes)
};
